...
def prepare_sepia(countries):
 ...
 LOCAL_GAS = LOCAL_GAS.drop('Unnamed: 7', axis=1)
 LOCAL_OIL = CONFIG["OIL_PRO"]
 LOCAL_OIL = LOCAL_OIL.drop('Unnamed: 7', axis=1)
 ...
 for country in ALL_COUNTRIES:
    ...
    data = pd.read_excel(datafile, sheet_name="Inputs", index_col=0, usecols="C:I")
    data.reset_index(drop=True, inplace=False)
    data=data.T
    
    
    '''load co2 input data for Sepia'''
    data_co2 = pd.read_excel(datafile, sheet_name="Inputs_co2", index_col=0, usecols="C:I")
    ...
    for en_code in ['ura']:
      value_loss_nuc_2025 = flows.loc['2025', (en_code + '_pe', 'elc_se', 'thm')].sum() * nuc_los
      value_loss_nuc_2030 = flows.loc['2030', (en_code + '_pe', 'elc_se', 'thm')].sum() * nuc_los
      value_loss_nuc_2035 = flows.loc['2035', (en_code + '_pe', 'elc_se', 'thm')].sum() * nuc_los
      value_loss_nuc_2040 = flows.loc['2040', (en_code + '_pe', 'elc_se', 'thm')].sum() * nuc_los
      value_loss_nuc_2045 = flows.loc['2045', (en_code + '_pe', 'elc_se', 'thm')].sum() * nuc_los
      value_loss_nuc_2050 = flows.loc['2050', (en_code + '_pe', 'elc_se', 'thm')].sum() * nuc_los
      flows.loc['2025', (en_code + '_pe', 'per', 'thm')] = value_loss_nuc_2025
      flows.loc['2030', (en_code + '_pe', 'per', 'thm')] = value_loss_nuc_2030
      flows.loc['2035', (en_code + '_pe', 'per', 'thm')] = value_loss_nuc_2035
      flows.loc['2040', (en_code + '_pe', 'per', 'thm')] = value_loss_nuc_2040
      flows.loc['2045', (en_code + '_pe', 'per', 'thm')] = value_loss_nuc_2045
    ...
    if country != 'EU':     
     for en_code in ['fgs']:
        ...
        flows_co2.loc['2025', (en_code + '_ghg', 'gas_ghg', '')] = values_gas_emm['2025'] * co2_intensity_gas
        flows_co2.loc['2030', (en_code + '_ghg', 'gas_ghg', '')] = values_gas_emm['2030'] * co2_intensity_gas
        flows_co2.loc['2035', (en_code + '_ghg', 'gas_ghg', '')] = values_gas_emm['2035'] * co2_intensity_gas
 ...
 def generate_results(flows, tot_results, country):
    ...
    ghg_sector_cum.loc['2030'] *= 5
    ghg_sector_cum.loc['2035'] *= 5
    ghg_sector_cum.loc['2040'] *= 5
    ghg_sector_cum.loc['2045'] *= 5
    ghg_sector_cum.loc['2050'] *= 5
    #multiplying by 10 for cumulative emissions
    ghg_source_cum = ghg_source.copy()
    ghg_source_cum.loc['2030'] *= 5
    ghg_source_cum.loc['2035'] *= 5
    ghg_source_cum.loc['2040'] *= 5
    ghg_source_cum.loc['2045'] *= 5
    ghg_source_cum.loc['2050'] *= 5
    ...
    if sepia_plots["Cumulative CO2 emissions by sector"] == True:
     ...
     html_items_emissions['MAIN'] += saf.combine_charts([('',saf.cumul(ghg_sector_cum, 2025))], MAIN_PARAMS, NODES,'', 'ghgchart',  results_xls_writer, 'MtCO<sub>2</sub>eq')
    else:
     saf.combine_charts([('',saf.cumul(ghg_sector_cum, 2025))], MAIN_PARAMS, NODES,'', 'ghgchart',  results_xls_writer, 'MtCO<sub>2</sub>eq')
    if sepia_plots["Cumulative CO2 emissions by source"] == True:
     ...
     html_items_emissions['MAIN'] += saf.combine_charts([('',saf.cumul(ghg_source_cum,2025))], MAIN_PARAMS, NODES, '','ghgchart', results_xls_writer, 'MtCO<sub>2</sub>')
    else:
     saf.combine_charts([('',saf.cumul(ghg_source_cum,2025))], MAIN_PARAMS, NODES, '','ghgchart', results_xls_writer, 'MtCO<sub>2</sub>')
    # Sankeys
