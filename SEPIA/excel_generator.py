...
def process_network(cluster, opt, sector_opt, planning_horizon, country):
        ...
        energy_demand = pd.read_csv(
            f"resources/{study}/energy_totals.csv",
            index_col=0)
        ...
        if country != 'EU':
          ...
          for i in range(max_i):
             ...
             ).loc[country_links].div(
                 1e6
             )  # TWh
             n.links[f"carrier_bus{i}"] = (
            n.links.loc[country_links, f"bus{i}"]
            .map(n.buses.carrier)
        )
        else:
          for i in range(max_i):
...
entries_to_select = ['solar', 'solar rooftop', 'onwind','solar-hsat',
                      ...
                      'H2 Electrolysis', 'methanolisation', 'Haber-Bosch', 'SMR', 'SMR CC', 'coal_2','solar-onwind Electrolysis',
                      'lignite_2','solar Electrolysis','onwind Electrolysis','offwind-ac Electrolysis','offwind-dc Electrolysis',
                      'CCGT_2', 'urban central solid biomass CHP_3', 'H2 Electrolysis_3', 'methanolisation_2',
                      'Haber-Bosch_2','H2 Electrolysis_2','FLC Electrolysis','FLC Electrolysis_2',
...
entry_label_mapping = {
    ...
    'solar Electrolysis':{'label': 'solar connected electrolyser', 'source': 'TWh', 'target': 'proesolh'},
    'onwind Electrolysis':{'label': 'onwind connected electrolyser', 'source': 'TWh', 'target': 'proeonh'},
    'offwind-ac Electrolysis':{'label': 'offwind-ac connected electrolyser', 'source': 'TWh', 'target': 'proeoffach'},
    'offwind-dc Electrolysis':{'label': 'offwind-dc connected electrolyser', 'source': 'TWh', 'target': 'proeoffdch'},
    'solar-onwind Electrolysis':{'label': 'hybrid electrolyser', 'source': 'TWh', 'target': 'pregenvrehyd'},
    'vre H2 Electrolysis_2':{'label': 'vre connected electrolyser losses', 'source': 'TWh', 'target': 'proesolhlos'},
    ...
    'FLC Electrolysis': {'label': 'Production of H2 via electrolysis', 'source': 'TWh', 'target': 'prohydflc'},
    ...
    'FLC Electrolysis_3': {'label': 'Transformation losses (electrolysis)', 'source': 'TWh', 'target': 'losshelyflc'},
    'FLC Electrolysis_2': {'label': 'electrolysis to heat', 'source': 'TWh', 'target': 'prehvvapflc'},
...
def prepare_emissions(cluster, opt, sector_opt,planning_horizon, country):
        ...
        if "p3" in n.links_t:
          if country == 'EU':
            ...
            value = -(
                n.snapshot_weightings.generators
                @ n.links_t.p3[
                    [c for c in n.links_t.p3.columns
                     if c.startswith(country) and "urban central gas CHP" in c]
                ]
            ).sum()
          collection.append(
                pd.Series(
        ...
        if "p4" in n.links_t:
            ...
            if country == 'EU':
                ...
                value = -(
                    ...
                    @ n.links_t.p3[
                        [c for c in n.links_t.p3.columns
                         if c.startswith(country) and tech in c]
                    ]
                ).sum()
            collection.append(
            ...
            if country == 'EU':
                ...
                value = -(
                    n.snapshot_weightings.generators
                    @ n.links_t.p4[
                        [c for c in n.links_t.p4.columns
                         if c.startswith(country) and tech in c]
                    ]
                ).sum()
            collection.append(
        ...
        if country == 'EU':
            ...
            value = -(
                n.snapshot_weightings.generators
                @ n.links_t.p0[
                    [c for c in n.links_t.p0.columns
                     if c.startswith(country) and "urban central solid biomass CHP" in c]
                ]
            ).sum()
        collection.append(
            pd.Series(
        ...
        if country == 'EU':
            value = -(n.snapshot_weightings.generators @ n.links_t.p3.filter(like="biomass-to-methanol CC")).sum().sum() 
        else:
            value = -(n.snapshot_weightings.generators @ n.links_t.p3.filter(like="biomass-to-methanol CC").filter(
                like=country)).sum().sum() 
        collection.append(
            pd.Series(
                dict(label="biomass-to-methanol CC", source="methanol", target="co2 stored",
                     value=value)
            )
        )
        ...
        # LULUCF
        # LULUCF = (
        #     pd.read_csv(f"resources/{study}/co2_totals.csv", index_col=0)).T
...
entries_to_select_c = ['process emissions', 'process emissions CC', 'CCGT', 'lignite',
                       ...
                       'rural biomass boiler_2','biomass-to-methanol CC',
...
entry_label_mapping_c = {
    ...
    'biomass-to-methanol CC': {'label': 'biomass-to-methanol CC', 'source': 'MtCO2', 'target': 'emmbmmetstm'},
}
def write_to_excel(cluster, opt, sector_opt, planning_horizons,countries, filename):
    ...
    for country in countries:
        ...
        with pd.ExcelWriter(country_filename, engine='openpyxl') as writer:
            ...
            selected_entries_df = selected_entries_df[['label', 'source', 'target','2025', '2030','2035', '2040','2045', '2050']]
            ...
            total_df = total_df.groupby(['label', 'source', 'target'], as_index=False)[['2025', '2030','2035', '2040','2045', '2050']].sum()
    ...
    for country in countries:
        ...
        with pd.ExcelWriter(country_filename, engine='openpyxl', mode='a',
            ...
            selected_entries_cf = selected_entries_cf[['label', 'source', 'target','2025', '2030','2035', '2040','2045', '2050']]
            total_cf = selected_entries_cf
            for year in ['2025', '2030','2035', '2040','2045', '2050']:
              if year in total_cf.columns:
                total_cf[year] = total_cf[year].fillna(0)
            total_cf = total_cf.groupby(['label', 'source', 'target'], as_index=False)[['2025', '2030','2035', '2040','2045', '2050']].sum()
